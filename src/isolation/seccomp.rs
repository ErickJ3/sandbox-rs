...
mod tests {
    ...
    #[test]
    fn test_seccomp_filter_comparison() {
        let minimal = SeccompFilter::minimal();
        let compute = SeccompFilter::from_profile(SeccompProfile::Compute);

        // Compute should have at least all minimal syscalls
        for syscall in minimal.allowed_syscalls() {
            if !minimal.blocked_syscalls().contains(syscall) {
                // Most minimal should be in compute, but let's just check it doesn't error
